...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop Talisman server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Talisman server stopping";
}
...
{ //  category              name                      actor (function)         okSafeMode threadSafe reqWallet
  //  --------------------- ------------------------  -----------------------  ---------- ---------- ---------
    ...
    /* Talisman features */
    { "talisman",               "masternode",             &masternode,             true,      true,       false },
    { "talisman",               "masternodelist",         &masternodelist,         true,      true,       false },
    { "talisman",               "mnbudget",               &mnbudget,               true,      true,       false },
    { "talisman",               "mnbudgetvoteraw",        &mnbudgetvoteraw,        true,      true,       false },
    { "talisman",               "mnfinalbudget",          &mnfinalbudget,          true,      true,       false },
    { "talisman",               "mnsync",                 &mnsync,                 true,      true,       false },
    { "talisman",               "spork",                  &spork,                  true,      true,       false },
#ifdef ENABLE_WALLET
    { "talisman",               "darksend",               &darksend,               false,     false,      true  }, /* not threadSafe because of SendMoney */
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use talismand, or the -server option to talisman-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
...
std::string HelpExampleCli(string methodname, string args){
    return "> talisman-cli " + methodname + " " + args + "\n";
}
